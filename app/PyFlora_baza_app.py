from PYFlora_baza_repozitorij import (
    Korisnik,
    Biljke,
    PyPosude,
    SQLAlchemyRepozitorij,
    spoji_se_na_bazu,
)

### OVAJ MODUL SE KORISTI ZA RAD S BAZOM IZVAN GUI APLIKACIJE
...
def spremi_biljku_u_bazu(repozitorij):
    ...
    moja_biljka = Biljke(
        ime_biljke=zeljena_biljka,
        slika_biljke=putanja_do_biljke,
...
def spremi_posudu_u_bazu(repozitorij):
    ...
    moja_posuda = PyPosude(
        ime_posude=ime_posude,
        posadena_biljka=posadena_biljka,
        slika_posude=slika_posude,
    )
...
def kreiraj_korisnika_koji_ne_postoji(repozitorij):
    ...
    if not user:
        lozinka = input("Unesite lozinku: ")
        user = Korisnik(username=korisnicno_ime, password=lozinka)
        # user.ispisi_podatke()
        ...
        print(
            f"Djelatnik sa korisnickim imenom '{korisnicno_ime.upper()}' postoji u bazi"
        )
...
def pokreni_aplikaciju(ime_baze):
    ...
    # repozitorij.get_all_users()
    #kreiraj_korisnika(repozitorij)
    repozitorij.izbrisi_pyposudu(6)
